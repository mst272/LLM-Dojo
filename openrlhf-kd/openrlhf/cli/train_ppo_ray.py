...
if __name__ == "__main__":
    ...
    parser.add_argument("--gamma", type=float, default=1, help="Discount factor for return computation")
    ...
    parser.add_argument(
        "--advantage_estimator",
        type=str,
        choices=["reinforce", "rloo", "reinforce_baseline", "group_norm", "dr_grpo", "kd"],
        default="reinforce",
        help="Choose advantage estimation method: reinforce, rloo, reinforce_baseline, group_norm, dr_grpo, kd (knowledge distillation)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="KD blending coefficient for hybrid KD+RLVR mode. "
        "0.0 = pure RLVR, 1.0 = pure KD for KD datasources. "
        "Only effective when advantage_estimator != 'kd'. "
        "Final advantage = (1-kd_coef)*rlvr_adv + kd_coef*kd_adv for KD datasources.",
    )
    ...
    assert args.remote_rm_url, "Local reward model is disabled. Please set --remote_rm_url or --agent_func_path."
