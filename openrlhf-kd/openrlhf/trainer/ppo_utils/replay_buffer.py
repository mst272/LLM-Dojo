...
class BufferItem:
    ...
    info: Optional[dict]
    datasource: Optional[str] = None  # data source identifier for per-datasource reward monitoring


def split_experience_batch(experience: Experience) -> List[BufferItem]:
    """Split a batch of experiences into individual BufferItems."""
    batch_size = len(experience.sequences)
    # Get fields from BufferItem, excluding 'info' and 'datasource' (handled separately)
    keys = tuple(field.name for field in fields(BufferItem) if field.name not in ("info", "datasource"))
    ...
    for i in range(batch_size):
        ...
        # Process datasource
        if experience.datasources and len(experience.datasources) == batch_size:
            item["datasource"] = experience.datasources[i]
        else:
            item["datasource"] = "default"
...
def make_experience_batch(items: List[BufferItem], packing_samples=False) -> Experience:
    ...
    # Process datasources
    kwargs["datasources"] = [item.datasource if item.datasource else "default" for item in items]
...
def remove_padding_in_sequences(items):
    for item in items:
        ...
        # Remove right padding for all tensors (excluding 'info' and 'datasource')
        keys = tuple(field.name for field in fields(BufferItem) if field.name not in ("info", "datasource"))
        for key in keys:
