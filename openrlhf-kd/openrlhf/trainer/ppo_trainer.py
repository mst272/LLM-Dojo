...
class BasePPOTrainer(ABC):
    ...
    def _init_swanlab(self):
        # swanlab/tensorboard setting
        self._swanlab = None
        self._tensorboard = None
        self.generated_samples_table = None
        if self.strategy.args.use_swanlab:
            import swanlab
            ...
            swanlab.init(
                workspace=self.strategy.args.swanlab_workspace,
                project=self.strategy.args.swanlab_project,
                experiment_name=self.strategy.args.swanlab_run_name,
                config=self.strategy.args.__dict__,
            )

            #self.generated_samples_table = swanlab.Table(columns=["global_step", "text", "reward"])

        # Initialize TensorBoard writer if swanlab is not available
        if self.strategy.args.use_tensorboard and self._swanlab is None:
            from torch.utils.tensorboard import SummaryWriter

            os.makedirs(self.strategy.args.use_tensorboard, exist_ok=True)
            log_dir = os.path.join(self.strategy.args.use_tensorboard, self.strategy.args.swanlab_run_name)
            self._tensorboard = SummaryWriter(log_dir=log_dir)
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        if global_step % args.logging_steps == 0:
            ...
            if self._swanlab is not None:
                # Add generated samples to swanlab using Table
                if "generated_samples" in logs_dict:
                    ...
                    if self.generated_samples_table is not None:
                        self.generated_samples_table.add_data(global_step, *logs_dict.pop("generated_samples"))
                        self._swanlab.log({"train/generated_samples": self.generated_samples_table})
                    else:
                        logs_dict.pop("generated_samples")
                logs = {
                    "train/%s" % k: v
                    for k, v in {
                        **logs_dict,
    ...
    def evaluate(self, eval_dataloader, global_step, temperature=0.6, n_samples_per_prompt=1):
        ...
        with torch.no_grad():
           # # First collect all prompts and labels
            # First collect all prompts, labels, and datasources
            all_prompts = []
            all_labels = []
          #  prompt_to_datasource = {}  # Dictionary to store mapping between prompts and their data sources
            ...
            # )
                all_prompts, all_labels, all_datasources, remote_reward_model=self.remote_reward_model, **generate_kwargs
            )

            # duplicate prompts and labels for each sample
            # duplicate prompts, labels and datasources for each sample
            all_prompts = sum([s.prompts for s in samples_list], [])
            all_labels = sum([s.labels for s in samples_list], [])
            all_datasources_expanded = sum([s.datasources for s in samples_list], [])
            ...
            for i in range(num_prompts):
                ...
                # Get the datasource (first one in the chunk, all samples in chunk have same datasource)
                datasource = all_datasources_expanded[i * n_samples_per_prompt]
    ...
    def fit(
        ...
        for episode in range(episode, args.num_episodes):
           ...
           # for _, rand_prompts, labels in self.prompts_dataloader:
            for datasources, rand_prompts, labels in self.prompts_dataloader:
                ...
                rollout_samples = self.samples_generator.generate_samples(
                    ...
                    rand_prompts, labels, datasources, remote_reward_model=remote_reward_model, **self.generate_kwargs
                )
                pbar.update()
                ...
                # Compute per-datasource reward statistics from experiences
                datasource_rewards = {}
                ...
                for exp in experiences:
                    ...
                    if ds_len > 0 and has_rewards:
                        total_samples_with_ds += ds_len
                        rewards = exp.rewards.tolist() if hasattr(exp.rewards, 'tolist') else list(exp.rewards)
                        for ds, reward in zip(exp.datasources, rewards):
                            if ds not in datasource_rewards:
                ...
                if datasource_rewards:
                    ...
                    for ds, ds_rewards in datasource_rewards.items():
                        ds_mean = sum(ds_rewards) / len(ds_rewards)
                        ds_passed = sum(1 for r in ds_rewards if r > 0)
                        logger.info(f"   - {ds}: count={len(ds_rewards)}, passed={ds_passed}, mean_reward={ds_mean:.4f}")
                        status[f"reward_{ds}"] = ds_mean
                        status[f"count_{ds}"] = len(ds_rewards)
                        status[f"passed_{ds}"] = ds_passed
                else:
                    logger.warning("⚠️ No datasource information found in experiences!")
