    ...
    def ppo_train(self, kl_ctl: float):
        ...
        for epoch in range(self.max_epochs):
            ...
            for step, experience in enumerate(pbar):
                ...
                # NOTE:
                # For mixed datasources, some mini-batches may not contain a given datasource at all.
                # If we simply average `pass_rate_{ds}` across mini-batches, those "missing" mini-batches
                # contribute zeros and will dilute the true pass rate.
                # We therefore track the (global) mini-batch size and later aggregate pass_rate using:
                #   pass_rate_{ds} = sum(reward_sum_{ds} * global_batch_size) / sum(ratio_{ds} * global_batch_size)
                # where `reward_sum_{ds}` and `ratio_{ds}` are per-sample means for the mini-batch.
                local_batch_size = int(experience.sequences.shape[0]) if experience.sequences is not None else 0
                global_batch_size = self.strategy.all_reduce(local_batch_size, op="sum")
                ...
                for ratio_key in ratio_keys:
                    ...
                    reward_sum_key = f"reward_sum_{ds}"
                    if reward_sum_key not in status:
                        continue
                    ratio_val = status[ratio_key]
                    reward_sum_val = status[reward_sum_key]
                    if ratio_val > 0:
                        true_pass = reward_sum_val / (ratio_val + 1e-8)
                    else:
                        true_pass = 0.0
                    #status[f"pass_rate_true_{ds}"] = true_pass
                    status[f"pass_rate_{ds}"] = true_pass
        ...
        if status_list:
            # Default: simple mean across mini-batches for most scalar metrics.
            ...
            # For per-datasource pass rates: re-aggregate using ratio/reward_sum weighted by global_batch_size
            # to avoid dilution from mini-batches where that datasource is absent.
            ratio_keys = [k for k in status_mean.keys() if k.startswith("ratio_")]
            for ratio_key in ratio_keys:
                ds = ratio_key[len("ratio_") :]
                reward_sum_key = f"reward_sum_{ds}"
                pass_rate_key = f"pass_rate_{ds}"
                if reward_sum_key not in status_mean:
                    continue

                sum_reward = 0.0
                sum_count = 0.0
                for st in status_list:
                    gb = float(st.get("global_batch_size", 1.0))
                    sum_reward += float(st.get(reward_sum_key, 0.0)) * gb
                    sum_count += float(st.get(ratio_key, 0.0)) * gb

                if sum_count > 0:
                    status_mean[pass_rate_key] = sum_reward / (sum_count + 1e-8)
                else:
                    # No samples of this datasource in the whole PPO update.
                    # Keep the curve continuous by logging 0 when ratio==0.
                    status_mean[pass_rate_key] = 0.0
        return status_mean
